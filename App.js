...
//            c = d3.scaleOrdinal(d3.schemeCategory20).domain(d3.range(n));
        ...
        gApp._nodes.forEach(function(node){
                    ...
                    callback: function (records, operation, success) {
                        if (success) {
                            _.each(records, function(record){
                                ...
                                console.log( 'Succ: ', node.record.data.FormattedID, i, 'Pred: ', record.data.FormattedID, j);
            ...
            if (node.record && node.record.data.Successors && node.record.data.Successors.Count){
                console.log('Getting succs for: ', node.record.data.FormattedID);
                //Now we need to find the Predecessor details and dynamically fill in the matix
                var cnf = Ext.clone(
                {
                    fetch: true,
                    callback: function (records, operation, success) {
                        if (success) {
                            _.each(records, function(record){
                                ...
                                console.log( 'Pred: ', node.record.data.FormattedID, i, 'Succ: ', record.data.FormattedID, j);
    ...
    _sortOrders: {
        ...
        Release:   function() {
            return d3.range(gApp._nodes.length).sort(function(a, b) { 
                var a = gApp._nodes[a].record.get('Release') ? gApp._nodes[a].record.get('Release')._refObjectName : gApp.NOT_SET_STRING;
                var b = gApp._nodes[b].record.get('Release') ? gApp._nodes[b].record.get('Release')._refObjectName : gApp.NOT_SET_STRING;
...
//                            gApp._getArtifacts(ptype);                            
    ...
    _findNodeIndexByRef: function(nodes, ref) {
        var retval = -1;
        retval = _.findIndex(nodes, function(node) {
